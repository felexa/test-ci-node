...
class Presenter {
     ...
     * @returns {Array}
     */
    getTabs(currentTabName) {
        let result = this._buildTabs();

        this._setActiveTabByName(
            result,
            this._hasTab(currentTabName) ? currentTabName : this._getDefaultTabName()
        );
     ...
     * @return {Presenter}
     */
    getFaq(alias, success, error) {
        if (_.isFunction(success) && _.isFunction(error)) {
            this.Model.getFaq(alias, success, error);
        }
